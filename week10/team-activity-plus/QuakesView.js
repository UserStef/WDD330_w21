...
export default class QuakesView {
    renderQuakeList(quakeList, listElement) {
        ...
        listElement.innerHTML = quakeList.features
            .map(quake => {
                return `<li id="id_${quake.id}" data-id="${quake.id}" class="list">
                <span class="place" data-id="${quake.id}">${quake.properties.title},</span>&nbsp;
                <span class="time">${new Date(quake.properties.time)}</span></li>`;
            })
            .join('');
    }
